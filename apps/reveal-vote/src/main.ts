...
async function bootstrap() {
    ...
    //NOTE - Cấu hình CORS
    app.enableCors({
        origin: CONFIGURATION.REVEAL_VOTE_CONFIG.CORS_ORIGINS.split(','),
    ...
    app.connectMicroservice<MicroserviceOptions>(
        {
            ...
            options: {
                host: CONFIGURATION.REVEAL_VOTE_CONFIG.TCP_HOST,
                port: CONFIGURATION.REVEAL_VOTE_CONFIG.TCP_PORT,
    ...
    const globalPrefix = CONFIGURATION.REVEAL_VOTE_CONFIG.HTTP_GLOBAL_PREFIX
    ...
    Logger.log(
        `🚀 TCP microservice is running on: ${CONFIGURATION.REVEAL_VOTE_CONFIG.TCP_HOST}:${CONFIGURATION.REVEAL_VOTE_CONFIG.TCP_PORT}`,
        'Bootstrap'
    )

    const swaggerConfig = new DocumentBuilder()
        .setTitle('Reveal Vote API')
        .setDescription('Reveal Vote API documentation')
        .setVersion('1.0')
        .addBearerAuth(
            {
                type: 'http',
    ...
    const port = CONFIGURATION.REVEAL_VOTE_CONFIG.HTTP_PORT
